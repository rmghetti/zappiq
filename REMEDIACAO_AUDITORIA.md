...
- **ONDA 5a CONCLUÍDA (05/07 madrugada):** 5 novas funcionalidades de UX, 760 testes verdes, build/tsc limpos, SEM migração nova (reusa colunas existentes). 5a.1 handoff completo na conversa (botões Assumir/Encerrar/Reabrir/Nota + banner "IA pausada" + Retomar Iza; rotas novas reopen/resume-ai), 5a.2 **playground "Testar minha IA"** (chat de teste contra prompt+RAG real, mostra QUAIS docs a IA usou — a ferramenta que teria denunciado o RAG morto no dia 1), 5a.3 drawer de detalhe/edição de deal (título/valor/contato/timeline/conversa), 5a.4 webchat vira lead no CRM (source=webchat, idempotente), 5a.5 opt-out automático (SAIR/PARAR → consentMarketing=false, LGPD/Meta).
- **ONDA 5b CONCLUÍDA (05/07 madrugada):** 5 inclusões finais, 827 testes verdes, 1 migração aditiva (`20260705000005_message_template_reengagement`). 5b.1 estado REAL da assinatura no billing (status/ciclo/trial countdown/faturas, honesto quando pré-Stripe), 5b.2 gestão de templates WhatsApp (/templates: CRUD + submeter à Meta + conceito de reengajamento 24h; campanha deixa de mostrar "nenhum template"; PUT endurecido), 5b.3 desconectar canal + saúde (limpa credencial por canal), 5b.4 automação de DSR (exportar JSON/CSV + anonimizar + e-mail ao titular), 5b.5 tela de Tarefas (lista as Tasks que a automação já criava + concluir).
- **✅ TODAS AS 5 ONDAS CONCLUÍDAS E (EM) PRODUÇÃO.** 36 defeitos da auditoria + 11 inclusões + limpeza. Suíte cresceu de 525 → 827 testes. Extras autônomos: RAG ressuscitado (credencial + URL + código + re-ingest de 60 chunks), 17 Agents criados, pipeline_stages das orgs faltantes (incl. CMJ). Build resiliente (tests fora do tsconfig de build).
